...
(fn setup []
  ...
  (set opt.winbar "%f")
  (set opt.laststatus 3) ;; Set global statusline for the entire screen.

  ;; Startify.
  ;; TODO: should be able to rewrite this myself?
  (set vim.g.startify_lists [{:type "dir"
  ...
  (let [zk-home (vim.fn.expand "~/zk")
    ...
    (rsetup :telekasten {:home      zk-home
  ...
  (create-autocmd "TextYankPost" {:group "Yank"
                                  :callback #(hl-on-yank)}))
