...
(local {: split : cut} (require :skr.std))
...
(fn json-pretty-print [json]
  ...
  (let [(out code) (-> {:command :jq :writer json}
                       (Job:new)
                       (: :sync))
        ok? (= code 0)]
    (if ok? out (error (.. "jq exited with code " code)))))
...
(fn write [buf {: query : resp}]
  ...
  (let [query-lines (split query "\n")
        ...
        lines (flatten [delim-query query-lines delim-resp resp-lines])]
...
(fn read-token []
  ...
  (fn read [file]
    ...
    (let [[body] (-> {:command :cat :args [file] :enable_recording true}
                     (Job:new)
                     (: :sync))]
      (json.decode body)))
  ...
  (fn refresh []
    ...
    (-> {:command :nclogin :args [:-stage :prod]}
        (Job:new)
        (: :sync)))
  ...
  (let [;; Expand the home directory for the locally cached file.
        ...
        {:id_token token :expires_at expiry} (read file)
...
(fn send-query [query]
  ...
  (let [;; Read token from local cache.
        token (read-token) ;; Construct query via curl.
...
(fn post []
  ...
  (let [buf (get-current-buf) ;; Parse the query from the buffer
        query (-> buf
